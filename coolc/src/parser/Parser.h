...
{
    PARSER_VERBOSE_ONLY(log_enter("parse_list"));
    ...
    while (_next_token.has_value() && _next_token->get_type() == expected_type)
    {
        ...
        {
            PARSER_VERBOSE_ONLY(log_exit("parse_list by cutout"));
            return true;
        }
    ...
    PARSER_VERBOSE_ONLY(log_exit("parse_list"));
    return true;
}
...
template <class T> std::shared_ptr<ast::Expression> Parser::parse_operator(const std::shared_ptr<ast::Expression> &lhs)
{
    ...
    PARSER_VERBOSE_ONLY(log_enter("parse_operators"));
    ...
    if (token_is_left_assoc_operator() || token_is_non_assoc_operator())
    {
        ...
        PARSER_VERBOSE_ONLY(log_exit("parse_operators for next operator"));
    ...
    PARSER_VERBOSE_ONLY(log_exit("parse_operators"));
