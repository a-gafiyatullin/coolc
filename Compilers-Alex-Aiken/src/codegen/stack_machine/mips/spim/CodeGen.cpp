...
CodeGen::CodeGen(const std::shared_ptr<semant::ClassNode> &root)
    : _root(root),
      _data(root),
      _asm(_code),
      _a0(_asm, Register::$a0),
      _s0(_asm, Register::$s0),
      _label_num(0),
      _equality_test_label(_asm, "equality_test", Label::ALLOW_NO_BIND), // don't check label binding during verification
      ...
      _genGC_assign(_asm, "_GenGC_Assign", Label::ALLOW_NO_BIND) // GC
{
    CODEGEN_FULL_VERBOSE_ONLY(_data.set_parent_logger(&_logger););
...
CodeBuffer CodeGen::emit()
{
    ...
    CODEGEN_FULL_VERBOSE_ONLY(init_table.set_parent_logger(&_logger););
...
void CodeGen::emit_class_code(const std::shared_ptr<semant::ClassNode> &node, SymbolTable &table)
{
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen class " + _current_class->_type->_string));
    ...
    {
        // it is real methods
        std::for_each(_current_class->_features.begin(), _current_class->_features.end(),
                      [&](const auto &feature)
                      {
    ...
    else
    {
        ...
        std::for_each(_current_class->_features.begin(), _current_class->_features.end(),
                      [&](const auto &feature)
                      {
                          ...
                          Label(_asm, DataSection::get_full_method_name(_current_class->_type->_string, feature->_object->_object), Label::ALLOW_NO_BIND);
                      });
    }

    // process children
    std::for_each(node->_children.begin(), node->_children.end(), [&](const auto &node)
                  { emit_class_code(node, table); });

    table.pop_scope();

    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen class " + _current_class->_type->_string));
}

void CodeGen::add_fields_to_table(const std::shared_ptr<ast::Class> &class_, SymbolTable &table)
{
    auto &code = _data.get_class_code(class_->_type);

    int fields_count = table.count();
    const int start_offset = fields_count * _WORD_SIZE + _OBJECT_HEADER_SIZE_IN_BYTES;

    // inherit parents fields types
    code.inherit_fields(_data.get_class_code(class_->_parent));

    // special case: String, Bool and Int. Don't create these fileds in table, because we wont use them
    if (semant::Semant::is_string(class_->_type))
    {
        code.set_prototype_field(semant::Semant::int_type());
        code.set_prototype_field(nullptr);
    }
    else if (semant::Semant::is_int(class_->_type) || semant::Semant::is_bool(class_->_type))
    {
        code.set_prototype_field(nullptr);
    }

    for (int i = 0; i < class_->_features.size(); i++)
    {
        if (std::holds_alternative<ast::AttrFeature>(class_->_features[i]->_base))
        {
            // save fields to local symbol table for further codegen
            table.add_symbol(class_->_features[i]->_object->_object, Symbol::FIELD, start_offset + (i * _WORD_SIZE));
            code.set_prototype_field(semant::Semant::exact_type(class_->_features[i]->_type, class_->_type));
            fields_count++;
        }
    }
}
...
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen binary expr"));
    ...
    if (!std::holds_alternative<ast::EqExpression>(expr._base))
    {
        ...
        std::visit(ast::overloaded{[&](const ast::MinusExpression &minus)
                                   ...
                                   {
                                       CODEGEN_FULL_VERBOSE_ONLY(assert(false && ("Impossible binary expression")));
                                   }},
                   expr._base);
    }
    else
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen binary expr"));
}

void CodeGen::emit_unary_expr(const ast::UnaryExpression &expr, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen unary expr"));
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen unary expr"));
}

void CodeGen::emit_expr(const std::shared_ptr<ast::Expression> &expr, SymbolTable &table)
{
    std::visit(ast::overloaded{[&](ast::BoolExpression &bool_expr)
                               {
                                   emit_bool_expr(bool_expr);
                               },
                               [&](ast::StringExpression &str)
                               {
                                   emit_string_expr(str);
                               },
                               [&](ast::IntExpression &number)
                               {
                                   emit_int_expr(number);
                               },
                               [&](ast::ObjectExpression &object)
                               {
                                   emit_object_expr(object, table);
                               },
                               [&](ast::BinaryExpression &binary_expr)
                               {
                                   emit_binary_expr(binary_expr, table);
                               },
                               [&](ast::UnaryExpression &unary_expr)
                               {
                                   emit_unary_expr(unary_expr, table);
                               },
                               [&](ast::NewExpression &new_)
                               {
                                   emit_new_expr(new_);
                               },
                               [&](ast::CaseExpression &case_)
                               {
                                   emit_cases_expr(case_, table);
                               },
                               [&](ast::LetExpression &let)
                               {
                                   emit_let_expr(let, table);
                               },
                               [&](ast::ListExpression &list)
                               {
                                   emit_list_expr(list, table);
                               },
                               [&](ast::WhileExpression &while_)
                               {
                                   emit_loop_expr(while_, table);
                               },
                               [&](ast::IfExpression &if_)
                               {
                                   emit_if_expr(if_, table);
                               },
                               [&](ast::DispatchExpression &dispatch)
                               {
                                   emit_dispatch_expr(dispatch, table);
                               },
                               [&](ast::AssignExpression &assign)
                               {
                                   emit_assign_expr(assign, table);
                               }},
               expr->_data);
}
...
{
    __ addiu(__ sp(), __ sp(), -(_WORD_SIZE * 3));
    __ sw(__ fp(), __ sp(), (_WORD_SIZE * 3));
    __ sw(_s0, __ sp(), (_WORD_SIZE * 2));
    __ sw(__ ra(), __ sp(), _WORD_SIZE);
    __ addiu(__ fp(), __ sp(), _WORD_SIZE * 3); // 4 for fp, 4 fo ra, 4 for s0
    __ move(_s0, _a0);                          // "this" in a0

    _asm.set_sp_offset(-(_WORD_SIZE * 3));
}

void CodeGen::emit_method_epilogue(const int &params_num)
{
    __ lw(__ ra(), __ sp(), _WORD_SIZE);
    __ lw(_s0, __ sp(), (2 * _WORD_SIZE));
    __ lw(__ fp(), __ sp(), (3 * _WORD_SIZE));
    __ addiu(__ sp(), __ sp(), (params_num + 3) * _WORD_SIZE); // pop all arguments to preserve stack
    __ jr(__ ra());

    _asm.set_sp_offset(0);
}

void CodeGen::emit_class_init_method(const std::shared_ptr<ast::Class> &class_, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen init for class " + _current_class->_type->_string));

    const AssemblerMarkSection mark(_asm, Label(_asm, DataSection::get_init_method_name(class_->_type->_string)));

    emit_method_prologue();

    if (class_->_parent->_string != class_->_type->_string) // Object moment
    {
        __ jal(Label(_asm, DataSection::get_init_method_name(class_->_parent->_string))); // receiver already is in acc, call parent constructor
    }
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen init for class " + _current_class->_type->_string));
}

void CodeGen::emit_class_method(const std::shared_ptr<ast::Feature> &method, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen method " + method->_object->_object));

    const AssemblerMarkSection mark(_asm, Label(_asm, DataSection::get_full_method_name(_current_class->_type->_string, method->_object->_object)));
    ...
    {
        table.add_symbol(params[i]->_object->_object, Symbol::LOCAL, (params_num - i) * _WORD_SIZE); // map formal parameters to arguments
    }
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen method " + method->_object->_object));
}
...
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen object expr for " + expr._object));

    if (!semant::Scope::can_assign(expr._object))
    {
        __ move(_a0, _s0); // self object: just copy to acc

        CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen object expr for " + expr._object));
        return;
    }
...
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen new expr"));
    ...
    else
    {
        ...
        __ sll(t5, t6, _OBJECT_HEADER_SIZE_IN_BYTES / _WORD_SIZE); // calculate offset in object table
        ...
        __ lw(t5, t5, _WORD_SIZE); // next slot is init method
        __ jalr(t5);
    }

    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen new expr"));
}

void CodeGen::emit_in_scope(const std::shared_ptr<ast::ObjectExpression> &object, const std::shared_ptr<ast::Type> &object_type,
                            const std::shared_ptr<ast::Expression> &expr, SymbolTable &table, const bool &assign_acc)
{
    table.push_scope();
    ...
    else
    {
        ...
        {
            __ la(_a0, DataSection::emit_init_value(_asm, _data, object_type));
            __ push(_a0);
        }
        else
        {
...
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen case expr"));

    emit_expr(expr._expr, table);

    std::string _case_branch_name = new_label_name();
    ...
    // in brackets because we want to deallocate t1 before next expressions emitting
    {
        ...
        __ bne(_a0, __ zero(), Label(_asm, _case_branch_name)); // is expression void
        // yes, it is void
    ...
    {
        const AssemblerMarkSection mark(_asm, Label(_asm, _case_branch_name));
        {
            const Register t1(_asm, Register::$t1);

            __ lw(t1, _a0, 0); // if we here, so object is in acc. Load its tag to t1

            _case_branch_name = (i < cases.size() - 1 ? new_label_name() : no_branch_name);
            __ blt(t1, _data.get_tag(cases[i]->_type->_string), Label(_asm, _case_branch_name));           // if object tag lower than the lowest tag for this branch, jump to next case
            __ bgt(t1, _data.get_max_child_tag(cases[i]->_type->_string), Label(_asm, _case_branch_name)); // if object tag higher that the highest tag for this branch, jump to next case
        }
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen case expr"));
}

void CodeGen::emit_let_expr(const ast::LetExpression &expr, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen let expr"));
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen let expr"));
}

void CodeGen::emit_list_expr(const ast::ListExpression &expr, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen list expr"));

    std::for_each(expr._exprs.begin(), expr._exprs.end(),
                  [&](const auto &e)
                  {
                      emit_expr(e, table); // last expression value will be in acc
                  });

    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen list expr"));
}
...
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen loop expr"));
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen loop expr"));
}

void CodeGen::emit_if_expr(const ast::IfExpression &expr, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen if expr"));
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen if expr"));
}
...
void CodeGen::emit_dispatch_expr(const ast::DispatchExpression &expr, SymbolTable &table)
{
    ...
    for (int i = 0; i < args_num; i++)
    {
        ...
        __ sw(_a0, __ sp(), (args_num - i) * _WORD_SIZE);
    }
    ...
    // not void
    std::visit(ast::overloaded{[&](const ast::ObjectDispatchExpression &disp)
                               {
                                   const int method_index = _data.get_class_code(semant::Semant::exact_type(expr._expr->_type, _current_class->_type))
                                                                .get_method_index(expr._object->_object);
                                   __ lw(t1, _a0, _DISPATCH_TABLE_OFFSET);   // load dispatch table
                                   __ lw(t1, t1, method_index * _WORD_SIZE); // load method label
                                   __ jalr(t1);                              // jump to method
                               },
                               [&](const ast::StaticDispatchExpression &disp)
                               {
                                   // we know exactly method name
                                   __ jal(Label(_asm, _data.get_class_code(disp._type).get_method_full_name(expr._object->_object)));
                               }},
               expr._base);
    const Label continue_label(_asm, new_label_name());
    __ j(continue_label);
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen dispatch expr"));
}

void CodeGen::emit_assign_expr(const ast::AssignExpression &expr, SymbolTable &table)
{
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_enter("gen assign expr"));
    ...
    CODEGEN_FULL_VERBOSE_ONLY(_logger.log_exit("gen assign expr"));
}

void CodeGen::emit_load_int(const Register &int_obj, const Register &int_val)
{
    __ lw(int_val, int_obj, _OBJECT_HEADER_SIZE_IN_BYTES);
}

void CodeGen::emit_store_int(const Register &int_obj, const Register &int_val)
{
    __ sw(int_val, int_obj, _OBJECT_HEADER_SIZE_IN_BYTES);
    emit_gc_update(int_obj, _OBJECT_HEADER_SIZE_IN_BYTES);
}
